	...
	if(hcan->Instance==CAN1)
	{
		...
		switch (rx_header.StdId)
		{
			...
			case CAN_3508_M7_ID: 
			{
				...
				if(i<2){
				can_bus.motor_LF[i].msg_cnt++ <= 50	?	
				can_bus.dji_encoder.get_moto_offset(&can_bus.motor_LF[i], rx_data_1): 
				can_bus.dji_encoder.get_motor_measure_Front(&can_bus.motor_LF[i], rx_data_1,i);
				//can_bus.dji_encoder.get_motor_measure(&can_bus.motor_LF[i], rx_data_1,i);
				break;
				}
				if(i>=2 && i<4){
				uint8_t j=i;
				i-=2;
				can_bus.motor_RF[i].msg_cnt++ <= 50	?	
				can_bus.dji_encoder.get_moto_offset(&can_bus.motor_RF[i], rx_data_1) : 
				can_bus.dji_encoder.get_motor_measure_Front(&can_bus.motor_RF[i], rx_data_1,j);
				//can_bus.dji_encoder.get_motor_measure(&can_bus.motor_RF[i], rx_data_1,j);
				break;
				}
				if(i>=4&&i<8){
				can_bus.motor_can1[i].msg_cnt++ <= 50	?	
				can_bus.dji_encoder.get_moto_offset(&can_bus.motor_can1[i], rx_data_1) : 
				can_bus.dji_encoder.get_motor_measure(&can_bus.motor_can1[i], rx_data_1,i);
				can_bus.dji_encoder.get_motor_measure(&can_bus.motor_can1[i], rx_data_1,i);
				break;
			}
		}
...
//			case CAN_3508_ALL_ID:
			{
				...
				i = rx_header.StdId - CAN_3508_M1_ID;
				if(i<2){
				can_bus.motor_RB[i].msg_cnt++ <= 50	?	
				can_bus.dji_encoder.get_moto_offset(&can_bus.motor_RB[i], rx_data_2) :  
				can_bus.dji_encoder.get_motor_measure_Behind(&can_bus.motor_RB[i], rx_data_2,i);
				//can_bus.dji_encoder.get_motor_measure(&can_bus.motor_RB[i], rx_data_2,i);
				break;
				}
				if(i>=2 && i<4){
				uint8_t j=i;
				i-=2;
				can_bus.motor_LB[i].msg_cnt++ <= 50	?	
				can_bus.dji_encoder.get_moto_offset(&can_bus.motor_LB[i], rx_data_2) : 
				can_bus.dji_encoder.get_motor_measure_Behind(&can_bus.motor_LB[i], rx_data_2,j);
				//can_bus.dji_encoder.get_motor_measure(&can_bus.motor_LB[i], rx_data_2,j);
				break;
				}
				if(i>=4&&i<8){
				can_bus.motor_can2[i].msg_cnt++ <= 50	?	
				can_bus.dji_encoder.get_moto_offset(&can_bus.motor_can2[i], rx_data_2) : 
				can_bus.dji_encoder.get_motor_measure(&can_bus.motor_can2[i], rx_data_2,i);
				can_bus.dji_encoder.get_motor_measure(&can_bus.motor_can2[i], rx_data_2,i);
				break;
				}
			}
...
void CAN_BUS::DJI_ENCODER::get_motor_measure(motor_measure_t *ptr,uint8_t data[],uint8_t id)                                                     
{
...
void CAN_BUS::DJI_ENCODER::get_motor_measure_Front(motor_measure_t *ptr,uint8_t data[],uint8_t id)                                                     
{
...
//				((ptr)->angle) = (int32_t)(((ptr)->ecd) - ((ptr)->last_ecd));
					...
					if(ptr->angle - ptr->last_angle > 4096)
						...
						ptr->round_cnt ++;
		if(id==0){
					ptr->total_angle = ptr->round_cnt * 8192 + ptr->angle - ptr->offset_angle-LF_0;
		}
		if(id==1){
					ptr->total_angle = ptr->round_cnt * 8192 + ptr->angle - ptr->offset_angle-LF_1;
		}
		if(id==2){
					ptr->total_angle = ptr->round_cnt * 8192 + ptr->angle - ptr->offset_angle-RF_2;
		}
		if(id==3){
					ptr->total_angle = ptr->round_cnt * 8192 + ptr->angle - ptr->offset_angle-RF_3;
		}			
					//ptr->total_angle = ptr->round_cnt * 8192 + ptr->angle - ptr->offset_angle;
}

void CAN_BUS::DJI_ENCODER::get_motor_measure_Behind(motor_measure_t *ptr,uint8_t data[],uint8_t id)                                                     
{
...
//				((ptr)->angle) = (int32_t)(((ptr)->ecd) - ((ptr)->last_ecd));
					...
					if(ptr->angle - ptr->last_angle > 4096)
						...
						ptr->round_cnt ++;
		if(id==0){
					ptr->total_angle = ptr->round_cnt * 8192 + ptr->angle - ptr->offset_angle-RB_0;
		}
		if(id==1){
					ptr->total_angle = ptr->round_cnt * 8192 + ptr->angle - ptr->offset_angle-RB_1;
		}
		if(id==2){
					ptr->total_angle = ptr->round_cnt * 8192 + ptr->angle - ptr->offset_angle-LB_2;
		}
		if(id==3){
					ptr->total_angle = ptr->round_cnt * 8192 + ptr->angle - ptr->offset_angle-LB_3;
		}			
					//ptr->total_angle = ptr->round_cnt * 8192 + ptr->angle - ptr->offset_angle;
