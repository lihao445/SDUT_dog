...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
  ...
  if(huart==&huart1)
  {
	for(uint8_t i=0;i<121;i++){
			if (ucRxBuffer[i]==0x50){ //数据头不对，则重新开始寻找0x55数据头 	
				if(ucRxBuffer[i+1]==0x03){

		sad=(uint16_t)ucRxBuffer[i+3]<<8|(uint16_t)ucRxBuffer[i+4];
		roll=sad/32768.0f*180.0f;
		jas=(uint16_t)ucRxBuffer[i+5]<<8|(uint16_t)ucRxBuffer[i+6];
		pitch=jas/32768.0f*180.0f;
		isa=(uint16_t)ucRxBuffer[i+7]<<8|(uint16_t)ucRxBuffer[i+8];
...
//	HAL_UART_Receive_IT(&huart1,ucRxBuffer,132);
	HAL_UART_Receive_DMA(&huart1,ucRxBuffer,121);
